...
| 卡5 策略与信号 | 两原型基类+策略包格式+orders CSV契约 | 玩具策略出合规信号文件 | **完成,待人类验收(2026-08-28)**:`strategies/{base,package,risk,context}.py` + 第一个策略包 `strategies/toy_lowvol/`(config/strategy/说明书,status=toy)+ `signals/{schema,run_signal}.py`(新鲜度红线);真实信号 `reports/2026-08-28_信号_toy_lowvol/orders_2026-08-26.csv`(20 只);回测与出信号同一 signal(),卡3 交叉验证数字不变;220 测试绿 |
