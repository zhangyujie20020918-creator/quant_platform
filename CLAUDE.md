...
> 第一次接手先读 `docs/使用指南.md`(傻瓜式:安装 → 数据 → 想法 → 因子 → 策略包 → 回测 → 信号 → 面板 → 示踪弹)。
...
- `.venv\Scripts\python -m streamlit run dashboard\app.py`(项目根目录):只读 reports/(原则7),四屏回测浏览器 / 因子 tear sheet / 信号 / 报告;
